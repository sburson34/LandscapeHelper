...
// Anonymous product-usage telemetry (shared Sburson.Shared.Telemetry).
// Scoped alongside the DbContext they depend on.
builder.Services.AddScoped<LandscapeHelper.Api.Services.Telemetry.TelemetryIngestService>();
builder.Services.AddScoped<LandscapeHelper.Api.Services.Telemetry.UsageDigestService>();
    ...
    if (db.Database.IsNpgsql())
    {
        db.Database.ExecuteSqlRaw(@"
            ...
            CREATE TABLE IF NOT EXISTS ""AnalyticsEvents"" (
                ...
                ""AnonId"" UUID NOT NULL,
                ""SessionId"" UUID NOT NULL,
                ""OccurredAt"" TIMESTAMP WITH TIME ZONE NOT NULL,
                ""ClientTs"" TIMESTAMP WITH TIME ZONE NOT NULL,
                ""PropsJson"" JSONB NULL,
                ""AppVersion"" VARCHAR(64) NULL,
                ""Platform"" VARCHAR(16) NULL
            );
            CREATE INDEX IF NOT EXISTS ""IX_AnalyticsEvents_AnonId_OccurredAt"" ON ""AnalyticsEvents""(""AnonId"", ""OccurredAt"");
...
// X-Admin-Token header). The app has no global admin middleware — admin checks
// are per-endpoint via RequireAdmin — so the gate is the sole guard here, matching
// the DIYHelper2 reference.
app.MapPost("/api/telemetry/events", async (
    Sburson.Shared.Telemetry.TelemetryBatchDto? body,
    LandscapeHelper.Api.Services.Telemetry.TelemetryIngestService ingest,
...
app.MapGet("/api/admin/usage-digest", async (
    HttpContext http,
    LandscapeHelper.Api.Services.Telemetry.UsageDigestService digest,
